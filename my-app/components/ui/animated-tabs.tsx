  ...
  content: (
    <div className="grid grid-cols-2 gap-4 w-full h-full">
      ...
      <div className="flex flex-col gap-y-2">
        <h2 className="text-2xl font-bold mb-0 text-black mt-0 !m-0">
          Tab 1
        </h2>
...
return (
  <div className={cn("w-full max-w-lg flex flex-col gap-y-1", className)}>
    <div className="flex gap-2 flex-wrap bg-[#eeeeee98] bg-opacity-50 backdrop-blur-sm p-1 rounded-xl">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => setActiveTab(tab.id)}
          className={cn(
            "relative px-3 py-1.5 text-sm font-medium rounded-lg text-black outline-none transition-colors"
          )}
        >
          {activeTab === tab.id && (
            <motion.div
              layoutId="active-tab"
              className="absolute inset-0 bg-[#eeeeeed1] bg-opacity-50 shadow-[0_0_20px_rgba(255,255,255,0.2)] backdrop-blur-sm !rounded-lg"
    ...
    <div className="p-4 bg-[#eeeeee98] shadow-[0_0_20px_rgba(255,255,255,0.2)] text-black bg-opacity-50 backdrop-blur-sm rounded-xl border min-h-60 h-full">
      {tabs.map(
        (tab) =>
          activeTab === tab.id && (
            <motion.div
